...
export default {
  ...
  watch: {
    "borrowInfo.amount"(value) {
      if (value > this.borrowAmount) {
        let that = this;
        this.$alert("Your don't have a sufficient borrow amount", {
          type: "error",
          callback() {
  ...
  methods: {
    getBorrowStatus() {
      this.$axios
        .$get("/api/core/borrowInfo/auth/getBorrowStatus")
        .then((response) => {
          this.borrowInfoStatus = response.data.status;
