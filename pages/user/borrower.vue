...
export default {
  data() {
    ...
    return {
      ...
      deleteUrl: BASE_API + "/api/oss/file/remove?url=",
      initUrl: BASE_API + "/api/core/dict/findByDictCode/",
      saveUrl: BASE_API + "/api/core/borrower/auth/save",
    };
  },
  created() {
    this.$axios
      ...
      .then((response) => {
        if (response.data.status == 0) {
          ...
          this.borrowerStatus = response.data.status;
          this.active = 2;
        }
      });
  },
  methods: {
    save() {
      ...
      this.$axios.$post(this.saveUrl, this.borrower).then((response) => {
        this.active = 1;
      });
    },
    onUploadRemove(file, fileList) {
      this.$axios
        .$delete(this.deleteUrl + file.response.data.URL)
        .then((response) => {
    ...
    initSelected() {
      this.$axios.$get(this.initUrl + "industry").then((response) => {
        this.industryList = response.data.list;
      });
      this.$axios.$get(this.initUrl + "education").then((response) => {
        this.educationList = response.data.list;
      });
      this.$axios.$get(this.initUrl + "income").then((response) => {
        this.incomeList = response.data.list;
      });
      this.$axios.$get(this.initUrl + "returnSource").then((response) => {
        this.returnSourceList = response.data.list;
      });
      this.$axios.$get(this.initUrl + "relation").then((response) => {
    ...
    onUploadSuccess(response, file, fileType) {
      if (response.code === 0) {
